<!DOCTYPE html>
<html lang="en" ng-app="myApp">
    <head>
        <meta charset="utf-8">
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <meta name="viewport" content="width=device-width, initial-scale=1">

        <!-- CSRF Token -->
        <meta name="csrf-token" content="{{ csrf_token() }}">

        <title>{{ config('app.name', 'Laravel') }}</title>

        <!-- Styles -->
        <link href="{{URL::asset('/css/app.css')}}" rel="stylesheet">
        <link href="{{URL::asset('angularjs/css/mystyles.css')}}" rel="stylesheet">
        <link href="{{URL::asset('/angularjs/css/bootstrap.css')}}" rel="stylesheet">
        <link href="{{URL::asset('/angularjs/css/bootstrap.min.css')}}" rel="stylesheet">
        <link href="{{URL::asset('/css/chatcss.css')}}" rel="stylesheet">

        <!-- Scripts -->
        <script>
            window.Laravel = <?php
echo json_encode([
    'csrfToken' => csrf_token(),
]);
?>
        </script>

                <script type="text/javascript" src="{{URL::asset('angularjs/js/jquery.min.js')}}"></script>
                <script type="text/javascript" src="{{URL::asset('angularjs/js/bootstrap.min.js')}}"></script>
                <script src="{{URL::asset('angularjs/js/angular.min.js')}}"></script>
                <script src="{{URL::asset('angularjs/js/angular-route.min.js')}}"></script>
                <script type="text/javascript" src="{{URL::asset('angularjs/modules/Modules.js')}}"></script>
                <script type="text/javascript" src="{{URL::asset('angularjs/js/route.js')}}"></script>
                <script type="text/javascript" src="{{URL::asset('angularjs/controllers/Controller.js')}}"></script>
                <script type="text/javascript" src="{{URL::asset('angularjs/js/bootstrap-ui.js')}}"></script>
                <script type="text/javascript" src="{{URL::asset('angularjs/js/ng-file-upload.js')}}"></script>
                <script type="text/javascript" src="{{URL::asset('angularjs/js/ng-file-upload-shim.js')}}"></script>
            </head>
            <body>
                <div id="app">
                    <nav class="navbar navbar-default navbar-static-top">
                        <div class="container">
                            <div class="navbar-header">

                                <!-- Collapsed Hamburger -->
                                <button type="button" class="navbar-toggle collapsed" data-toggle="collapse" data-target="#app-navbar-collapse">
                                    <span class="sr-only">Toggle Navigation</span>
                                    <span class="icon-bar"></span>
                                    <span class="icon-bar"></span>
                                    <span class="icon-bar"></span>
                                </button>

                                <!-- Branding Image -->
                                <a class="navbar-brand" href="{{ url('/') }}">
                                    {{ config('app.name', 'Laravel') }}
                                </a>
                            </div>

                            <div class="collapse navbar-collapse" id="app-navbar-collapse">
                                <!-- Left Side Of Navbar -->
                                <ul class="nav navbar-nav">
                                    &nbsp;
                                </ul>
                                @if (!Auth::guest()) <ul class="nav navbar-nav navbar-left">
                                    <li><a href="{{ url('/home') }}">Home</a></li>
                                    <li><a href="{{ url('/pay') }}">Pay</a></li>
                                    <li><a href="{{ url('/carusel') }}">Carusel</a></li>
                                    <li><a href="{{ url('/upload') }}">Upload</a></li>
                                    <li><a href="{{ url('/chat') }}">Chat</a></li>
                                @endif
                                </ul>
                                <!-- Right Side Of Navbar -->
                                <ul class="nav navbar-nav navbar-right">
                                    <!-- Authentication Links -->
                                    @if (Auth::guest())
                                    <li><a href="{{ url('/login') }}">Login</a></li>
                                    <li><a href="{{ url('/register') }}">Register</a></li>
                                    @else
                                   
                                    <li class="dropdown">
                                        <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-expanded="false">
                                            {{ Auth::user()->name }} <span class="caret"></span>
                                        </a>

                                        <ul class="dropdown-menu" role="menu">
                                            <li>
                                                <a href="{{ url('/logout') }}"
                                                   onclick="event.preventDefault();
                            document.getElementById('logout-form').submit();">
                                                    Logout
                                                </a>

                                                <form id="logout-form" action="{{ url('/logout') }}" method="POST" style="display: none;">
                                                    {{ csrf_field() }}
                                                </form>
                                            </li>
                                        </ul>
                                    </li>
                                    @endif
                                </ul>
                            </div>
                        </div>
                    </nav>
                    @yield('content')
                </div>

            </body>
        </html>
